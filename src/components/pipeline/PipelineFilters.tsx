    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {selectedProducts.map(productId => (
            <span key={productId} className="flex items-center gap-1 text-xs font-medium bg-amber-50 text-amber-700 border border-amber-100 px-2 py-0.5 rounded-full">
              {products.find(p => p.id === productId)?.name || productId}
              <span role="button" onClick={e => { e.stopPropagation(); onProductsChange(selectedProducts.filter(id => id !== productId)); }} className="cursor-pointer hover:opacity-70"><X size={11} /></span>
            </span>
          ))}
      ...
      >
        ...
        <div className="relative w-[180px] shrink-0">
          ...
          <select
            value={selectedProducts.length === 0 ? 'all' : selectedProducts.length === 1 ? selectedProducts[0] : 'multiple'}
            onChange={(e) => {
              ...
              if (val === 'all') {
                ...
                if (!selectedProducts.includes(val)) {
                  onProductsChange([...selectedProducts, val]);
                }
              }
            }}
            className={cn(
          ...
          >
            ...
            {selectedProducts.length > 1 && (
              <option value="multiple">{selectedProducts.length} produtos selecionados</option>
            )}
            {products.map((p: any) => (
              <option key={p.id} value={p.id}>
                {selectedProducts.includes(p.id) ? `✓ ${p.name}` : p.name}
              </option>
            ))}
          </select>
          <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={14} />
