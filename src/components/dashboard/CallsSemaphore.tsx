...
export function CallsSemaphore({ goals, isAdmin, currentUserId }: CallsSemaphoreProps) {
  ...
  return (
    <div 
      ref={containerRef} 
      className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex flex-col relative transition-all duration-300 ${
      ...
      <div className="flex flex-col gap-3 mb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className={`font-bold text-slate-800 dark:text-slate-200 ${isFullscreen ? 'text-2xl' : ''}`}>Ligações — Vendedores</h3>
            {loading && <Loader2 className="w-3.5 h-3.5 text-slate-400 animate-spin" />}
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleToggleFullscreen}
              className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800/50 transition-colors text-[11px] font-bold border border-slate-200 dark:border-slate-700 shadow-sm"
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] font-bold transition-colors ${
                showAll
                  ...
                  : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800/50'
              }`}
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center bg-slate-100 dark:bg-slate-800/50 rounded-xl p-1 gap-0.5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[11px] focus:outline-none focus:border-emerald-400"
                />
              </div>
              <div className="flex items-center gap-1.5">
              ...
              return (
                ...
                >
                  <div className="mb-2 flex flex-col items-center gap-1 opacity-80 group-hover:opacity-100">
                    <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm whitespace-nowrap">
                  ...
                  <div className="relative w-full flex flex-col items-center" style={{ height: `${CHART_HEIGHT}px` }}>
                    <div
                      className="absolute w-full max-w-[44px] border-t-2 border-dashed border-slate-300 dark:border-slate-600 z-10"
                    ...
                    >
                      ...
                      <div className="absolute inset-x-0 top-0 h-1/3 bg-white dark:bg-slate-900/10 rounded-t-2xl pointer-events-none" />
                    </div>
                    <div className="absolute bottom-0 w-full max-w-[44px] h-[2px] bg-slate-100 dark:bg-slate-800/50 rounded-full" />
                  </div>
                  ...
                  <div className="mt-1.5 text-center w-full px-1">
                    <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate uppercase tracking-tight group-hover:text-slate-800 dark:text-slate-200 transition-colors" title={s.user_name}>
                      {firstName}
                    </p>
                  </div>
                </div>
