...
export const SummaryTab: React.FC<{ kpis: any; pipelinePendingCount: number; pipelinePaidCount: number }> = ({ kpis, pipelinePendingCount, pipelinePaidCount }) => {
  return (
    <div className="divide-y divide-slate-100">
      <div className="p-6">
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-4">Saúde Financeira</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800">
            <p className="text-xs font-bold text-slate-400 uppercase mb-3">Composição de Receita</p>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600 dark:text-slate-400 font-medium">Alunos e Matrículas</span>
                <span className="text-sm font-bold text-emerald-600">R$ {fmt(kpis?.receita_total || 0)}</span>
              </div>
              <div className="pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                <span className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight">Total Geral</span>
