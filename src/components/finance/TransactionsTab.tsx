        ...
        <div className="overflow-x-auto min-h-[400px]">
          {isLoading && transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">Carregando transações...</p>
            </div>
          ) : filtered.length === 0 ? (
             <div className="flex flex-col items-center justify-center h-64">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50 dark:bg-slate-800/80 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-800 dark:text-slate-200">{t.description}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{(t as any).financial_categories?.name || 'Sem Categoria'} • Origem: {t.origin_type}</p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={cn(
                        ...
                        t.status === 'CANCELLED' && 'bg-slate-100 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400',
