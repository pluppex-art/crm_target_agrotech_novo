  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-slate-500 dark:text-slate-400">Taxas, impostos e deduções aplicados nos cálculos do DRE.</p>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white dark:bg-slate-900 rounded-xl border border-indigo-200 p-5 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <input required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Nome" className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border rounded-lg text-sm" />
            <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value as any }))} className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border rounded-lg text-sm">
              {Object.entries(typeLabel).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
            <input type="number" step="0.01" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: parseFloat(e.target.value) || 0 }))} className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border rounded-lg text-sm" />
          </div>
                ...
                return (
                  <tr key={r.id}>
                    <td className="px-5 py-3.5">
                      <p className="font-bold text-slate-700 dark:text-slate-300">{r.name}</p>
                    ...
                    <td className="px-5 py-3.5 text-xs text-slate-500 dark:text-slate-400">{ctxLabel[r.application_context]}</td>
