...
export function Integrations() {
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Integrações</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Conecte o CRM Target Agrotech com suas ferramentas favoritas.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {integrations.map((integration) => (
          <div key={integration.name} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group">
            <div className="flex items-start justify-between mb-4">
              <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 p-2 flex items-center justify-center">
              ...
              <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                integration.status === 'connected' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 dark:bg-slate-800/50 text-slate-400'
              }`}>
                {integration.status === 'connected' ? 'Conectado' : 'Desconectado'}
              </span>
            </div>
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-1">{integration.name}</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">{integration.description}</p>
            
            <button className={`w-full py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${
              integration.status === 'connected' 
                ? 'bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:bg-slate-700' 
