...
import { Activity, Plus, Trash2, Loader2, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useActivityCategoryStore } from '../../store/useActivityCategoryStore';

export function ManageActivityCategories() {
  const navigate = useNavigate();
  const { categories, fetchCategories, addCategory, deleteCategory, loading, subscribe } = useActivityCategoryStore();
    ...
    <div className="p-6 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleCreate} className="p-6 border-b border-slate-100">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 transition-all shadow-md shadow-blue-100 flex items-center gap-2 disabled:opacity-50"
            >
              <Plus size={18} />
              Adicionar
            </button>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl">
        <p className="text-xs text-blue-700 leading-relaxed font-medium">
          <strong>💡 Dica:</strong> As categorias de atividade classificam os registros criados em leads e tarefas, como <strong>Ligação</strong>, <strong>Visita</strong>, <strong>Reunião</strong>, entre outros.
        </p>
      </div>
    </div>
  );
