...
export const transactionService = {
  ...
  async getGanhoLeads(startDate: string, endDate: string): Promise<{
    ...
    const SELECT = 'id, name, created_at, value, product, discount, discount_type, discount_applied, valor_recebido, taxa_matricula_recebido, pix_completed, professor_proof_url';
    ...
    const [byStatus, byStage] = await Promise.all([
      ...
      supabase
        ...
        .eq('pipeline_stages.name', 'Ganho')
        .gte('created_at', startDate)
        .lte('created_at', endDate + 'T23:59:59'),
    ]);

    if (byStatus.error || byStage.error) {
      console.error('transactionService.getGanhoLeads:', byStatus.error ?? byStage.error);
      return [];
    }

    const seen = new Set<string>();
    const merged: { id: string; name: string; created_at: string; value: string | number; product: string | null; discount?: string | number; discount_type?: 'percent' | 'money'; discount_applied?: boolean; valor_recebido?: number | null; taxa_matricula_recebido?: number | null; pix_completed?: boolean; professor_proof_url?: string | null }[] = [];
    for (const row of [...((byStatus.data as any[]) ?? []), ...((byStage.data as any[]) ?? [])]) {
      if (!seen.has(row.id)) {
        seen.add(row.id);
