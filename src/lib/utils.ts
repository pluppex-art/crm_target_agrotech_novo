 ...
 */
export function getOccupancyData(turmas: Turma[], leads: Array<{ product?: string | null; product_id?: string | null }> = []): Array<{
  name: string;
  pct: number;
  level: 'red' | 'yellow' | 'green';
  alunos: number;
  ...
  return turmas
    ...
    .map((t, i) => {
      ...
      const leadsCount = leads.filter(l => l.product_id ? l.product_id === t.id : l.product === t.name).length;
      const cap = t.meta ?? 0;
